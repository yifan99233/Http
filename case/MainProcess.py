...
class Mprocess(object):
    ...
    def __init__( self , Lcookie = True, **data ):

        if Lcookie != True:
            self.http = Qur.Httpself(url = url )
        else:
            self.Lhttp = Qur.Httpself(url=data['url'], api=data['Lapi'], data=data['Ldata'])
            self.header = self.Lhttp.header
            self.cookie = self.Lhttp.cookie
    ...
    def XpsUpload ( self , time = 1 , mainto = False) :
